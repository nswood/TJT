...
from reweighting import weights_fillbkg, weights_fillsig, weights_fillflat, weights_1dsdmass
...
#totalData = totalData[totalData[:, 4] < 800]
...
np.random.seed(42)
weights, totalData = weights_1dsdmass(totalData, reweight_to='sig')
print(weights[:10])
print(totalData[:10])
weights, totalData = sklearn.utils.shuffle(weights, totalData)
print(weights[:10])
print(totalData[:10])
...
trainingWeights = weights[:trainingDataLength]
...
validationWeights = weights[trainingDataLength:trainingDataLength + validationDataLength]
...
for i in range(len(totalData)): 
    if totalData[i, -1] == 1:
        totalData_sig.append(totalData[i])
        weights_sig.append(weights[i])
    if totalData[i, -1] == 0:
        totalData_bkg.append(totalData[i])
        weights_bkg.append(weights[i])

plt.figure()
plt.hist(np.array(totalData_sig)[:, 5], bins=15, weights=weights_sig)
plt.title('Signal')
plt.xlabel('sdmass')
plt.savefig('2dhist_weights_sig.jpg')

plt.figure()
plt.hist(np.array(totalData_bkg)[:, 5], bins=15, weights=weights_bkg)
plt.title('Bkg')
plt.xlabel('sdmass')
plt.savefig('2dhist_weights_bkg.jpg')

print(weights[:50])
print(weights_sig[:50])
print(weights_bkg[:50])

if includeeventData:
    history = model.fit([particleTrainingData, eventTrainingData], trainingLabels, sample_weight=trainingWeights, epochs=numberOfEpochs, 
                        batch_size=batchSize, callbacks=modelCallbacks,
                    validation_data=([particleValidationData, eventValidationData], validationLabels, validationWeights))

else: 
    history = model.fit([particleTrainingData], trainingLabels, sample_weight=trainingWeights,
                    epochs=numberOfEpochs, batch_size=batchSize,
                    callbacks=modelCallbacks,
                    validation_data=([particleValidationData], validationLabels, validationWeights))
